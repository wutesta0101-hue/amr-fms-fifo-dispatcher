...
8. [結果與 KPI](#八結果與-kpi-分別利用改變策略及改變介面來度量)
9. [驗證方式](#九驗證方式)
...
### 介面換成 VDA5050 有沒有差異??
介面換成 VDA5050 後，端到端 KPI 並未見劣化：
12 項比較（3 策略 × 4 指標）中 **0 項超出雜訊變差**——4 項落在雜訊底線內、8 項優於基準。四組實驗共發出 **490 張 order**。因為8 項的 M2 側只有單次實驗（fifo、nearest 各只跑 r1），所以判定「改善」的 6 項指受限於實驗範圍。
...
### 比較3種策略，哪一種策略誰派得好
逐項數字、判斷準則與統計圖可參照：[`experiments/M3b-策略對照.md`](experiments/M3b-策略對照.md)
三種策略之間的差異遠大於雜訊：12 項兩兩比較中 11 項超出雜訊底線，只有「平均周轉 nearest vs rmf」較無差異。
