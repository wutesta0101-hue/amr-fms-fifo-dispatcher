...
bridge = rows('/tmp/m2bridge_*.jsonl')
vehicle = rows('/tmp/vda5050_tinyRobot*.jsonl')
...
if down and up:
    ...
    print(f'（每個任務約 10–13 段 → 單筆任務多出 {per_leg * 10:.0f}–{per_leg * 13:.0f}s）')

print('\n=== 每組的 order 段數 ===')
for path in sorted(glob.glob('/tmp/m2bridge_*.jsonl')):
    n = sum(1 for l in open(path, encoding='utf-8')
            if l.strip() and json.loads(l).get('event') == 'order_sent')
    print(f'  {path.split("m2bridge_")[1]:<20} {n} 張 order')
